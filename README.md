...
Tree를 Treasure로 바꾸고 싶어. Treasure는 청크가 로딩될 때 10% 확률로 청크 내의 무작위 위치에 스폰되거나, 적을 죽이면 5% 확률로 적이 죽은 자리에 스폰돼. Treasure에 플레이어가 닿으면 플레이어가 레벨업했을 때 4가지 능력치 선택지를 고르는 것처럼, 4가지 아티팩트 선택지 중 하나를 고를 수 있어. 아티팩트의 종류는 아티팩트1(플레이어 사이즈 25% 감소), 아티팩트2(플레이어 공격력 50% 증가), 아티팩트3(플레이어 공격속도 50% 증가), 아티팩트4(플레이어 이동속도 50% 증가), 아티팩트5(플레이어의 체력이 1초에 최대체력의 2%만큼 재생)
