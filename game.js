...
class PlasmaWeapon extends Weapon {
  ...
  update() {
    super.update();
    // 활성 빔 정리
    this.activeBeams = this.activeBeams.filter(beam => !beam.used);
  }
  ...
  applyLevelBonus() {
    ...
    switch(this.level) {
      ...
      case 4:
        ...
        break;
      case 5:
        this.baseCooldown *= 0.85;
        break;
      case 6:
        this.beamDuration += 200;
        break;
      case 7:
        this.damage += 20;
        break;
...
// 플라즈마 빔 클래스
class PlasmaBeam {
  constructor(x, y, angle, length, width, damage, duration) {
    this.x = x;
    this.y = y;
    this.angle = angle;
    this.length = length;
  ...
  update() {
    ...
    // 빔 경로상의 모든 적에게 데미지
    for (let enemy of gameObjects.enemies) {
      if (enemy.state !== 'moving' || this.hitEnemies.has(enemy)) continue;
      
      if (this.checkBeamCollision(enemy)) {
        enemy.takeDamage(this.damage);
  ...
  checkBeamCollision(enemy) {
    ...
    return dist <= (this.width / 2 + enemy.size);
  }
  ...
  draw(offsetX, offsetY) {
    const progress = (gameTimeSystem.getTime() - this.startTime) / this.duration;
    ...
    const endX = this.x + Math.cos(this.angle) * this.length + offsetX;
    const endY = this.y + Math.sin(this.angle) * this.length + offsetY;
    
    // 빔 외곽 글로우
    const glowWidth = this.width * (1 + Math.sin(this.pulseTime) * 0.2);
...
class MagneticWeapon extends Weapon {
  ...
  dealAreaDamage() {
    ...
    for (let enemy of gameObjects.enemies) {
      ...
      if (distance <= range) {
        const damage = this.tickDamage * player.getTotalRangedAttackPower();
...
// WeaponFactory에 새 무기 추가
const originalCreateWeapon = WeaponFactory.createWeapon;
WeaponFactory.createWeapon = function(type) {
  switch(type) {
    ...
    default:
      return originalCreateWeapon.call(this, type);
  }
};

// 합성 UI 열기/닫기
function toggleFusionUI() {
  fusionUIState.isOpen = !fusionUIState.isOpen;
  if (!fusionUIState.isOpen) {
    fusionUIState.slot1 = null;
    fusionUIState.slot2 = null;
  }
}

// 합성 UI 그리기
function drawFusionUI() {
  if (!fusionUIState.isOpen) return;
  ...
  ctx.font = '28px Arial';
  ctx.textAlign = 'center';
  ctx.fillText('무기 합성', canvas.width / 2, panelY + 40);
  
  // 합성 슬롯
  const slotSize = 100;
  const slotY = panelY + 100;
  const slot1X = panelX + 100;
  const slot2X = panelX + 250;
  const resultX = panelX + 450;
  
  // 슬롯 1
  drawFusionSlot(slot1X, slotY, slotSize, fusionUIState.slot1, '슬롯 1');
  
  // + 기호
  ctx.fillStyle = '#FFFFFF';
  ctx.font = '36px Arial';
  ctx.fillText('+', slot1X + slotSize + 40, slotY + slotSize/2 + 10);
  
  // 슬롯 2
  drawFusionSlot(slot2X, slotY, slotSize, fusionUIState.slot2, '슬롯 2');
  
  // = 기호
  ctx.fillText('=', slot2X + slotSize + 40, slotY + slotSize/2 + 10);
  
  // 결과 슬롯
  const result = (fusionUIState.slot1 && fusionUIState.slot2) ? 
    weaponFusionSystem.getFusionResult(fusionUIState.slot1.type, fusionUIState.slot2.type) : null;
  drawFusionResultSlot(resultX, slotY, slotSize, result);
  ...
  const canFuse = fusionUIState.slot1 && fusionUIState.slot2 && result;
  
  ctx.fillStyle = canFuse ? '#00FF00' : '#555555';
  ctx.fillRect(buttonX, buttonY, buttonWidth, buttonHeight);
  
  ctx.strokeStyle = canFuse ? '#FFFFFF' : '#333333';
  ctx.lineWidth = 2;
  ctx.strokeRect(buttonX, buttonY, buttonWidth, buttonHeight);
  
  ctx.fillStyle = canFuse ? '#000000' : '#999999';
  ctx.font = '20px Arial';
  ctx.textAlign = 'center';
  ctx.fillText('합성하기', canvas.width / 2, buttonY + 27);
  ...
  ctx.font = '24px Arial';
  ctx.fillText('X', closeX + 15, closeY + 20);
  
  // 보유 무기 목록
  const listY = panelY + panelHeight + 20;
  const weaponBoxSize = 80;
  const spacing = 10;
  
  ctx.fillStyle = '#FFFFFF';
  ctx.font = '18px Arial';
  ctx.textAlign = 'left';
  ctx.fillText('보유 무기 (드래그하여 슬롯에 배치)', panelX, listY - 5);
  
  let offsetX = 0;
  player.weapons.forEach((weapon, index) => {
    if (weapon.level === weapon.maxLevel) {
      const boxX = panelX + offsetX;
      const boxY = listY + 10;
      
      // 드래그 중이 아닌 경우에만 원래 위치에 표시
      if (fusionUIState.draggedWeapon !== weapon) {
        drawWeaponBox(boxX, boxY, weaponBoxSize, weapon);
      }
      
      offsetX += weaponBoxSize + spacing;
    }
  });
  
  // 드래그 중인 무기 그리기
  if (fusionUIState.draggedWeapon) {
    drawWeaponBox(
      mouseX - weaponBoxSize/2,
      mouseY - weaponBoxSize/2,
      weaponBoxSize,
      fusionUIState.draggedWeapon
    );
  }
}

// 합성 슬롯 그리기
function drawFusionSlot(x, y, size, weapon, label) {
  // 슬롯 배경
  ctx.fillStyle = weapon ? '#2a3f5f' : '#0b0c10';
  ...
  // 무기 표시
  if (weapon) {
    drawWeaponInSlot(x, y, size, weapon);
  }
}

// 결과 슬롯 그리기
function drawFusionResultSlot(x, y, size, resultType) {
  // 슬롯 배경
  ctx.fillStyle = resultType ? '#4a3f5f' : '#0b0c10';
  ctx.fillRect(x, y, size, size);
  
  // 슬롯 테두리
  ctx.strokeStyle = resultType ? '#FFD700' : '#666666';
  ctx.lineWidth = 3;
  ctx.strokeRect(x, y, size, size);
  
  // 라벨
  ctx.fillStyle = resultType ? '#FFD700' : '#999999';
  ctx.font = '14px Arial';
  ctx.textAlign = 'center';
  ctx.fillText('결과', x + size/2, y - 10);
  
  // 결과 표시
  if (resultType) {
    const resultName = resultType === 'plasma' ? '플라즈마 캐논' : '자기장';
    
    // 아이콘 (임시)
    ctx.fillStyle = resultType === 'plasma' ? '#FF00FF' : '#00FFFF';
    ctx.fillRect(x + 20, y + 20, size - 40, size - 40);
    
    // 이름
    ctx.fillStyle = '#FFFFFF';
    ctx.font = '16px Arial';
    ctx.fillText(resultName, x + size/2, y + size + 20);
  }
}

// 무기 박스 그리기
function drawWeaponBox(x, y, size, weapon) {
  // 배경
...
// 슬롯 내 무기 그리기
function drawWeaponInSlot(x, y, size, weapon) {
  // 무기 아이콘
  if (assetManager.loaded.weaponIcons && assetManager.images.weaponIcons[weapon.type]) {
    ctx.drawImage(
      assetManager.images.weaponIcons[weapon.type],
      x + 10, y + 10,
      size - 20, size - 20
    );
  }
  ...
  ctx.fillRect(x + size - 25, y + size - 25, 25, 25);
  
  ctx.fillStyle = '#FFD700';
  ctx.font = 'bold 14px Arial';
  ctx.textAlign = 'center';
  ctx.fillText(`${weapon.level}`, x + size - 12, y + size - 8);
  
  // 무기 이름
  ctx.fillStyle = '#FFFFFF';
  ctx.font = '12px Arial';
  ctx.fillText(getWeaponDisplayName(weapon.type), x + size/2, y + size + 15);
}

// 합성 UI 마우스 처리
function handleFusionUIMouseDown(e) {
  if (!fusionUIState.isOpen) return;
  
  const rect = canvas.getBoundingClientRect();
  const x = (e.clientX - rect.left) * (canvas.width / parseInt(canvas.style.width, 10));
  const y = (e.clientY - rect.top) * (canvas.height / parseInt(canvas.style.height, 10));
  
  // 보유 무기 목록에서 드래그 시작 확인
  const panelWidth = 600;
  const panelX = (canvas.width - panelWidth) / 2;
  const panelY = (canvas.height - 400) / 2;
  const listY = panelY + 420;
  const weaponBoxSize = 80;
  const spacing = 10;
  ...
  player.weapons.forEach(weapon => {
    if (weapon.level === weapon.maxLevel) {
      ...
      if (x >= boxX && x <= boxX + weaponBoxSize &&
          y >= boxY && y <= boxY + weaponBoxSize) {
        fusionUIState.draggedWeapon = weapon;
        fusionUIState.dragStartX = x;
        fusionUIState.dragStartY = y;
      }
...
function handleFusionUIMouseUp(e) {
  if (!fusionUIState.isOpen || !fusionUIState.draggedWeapon) return;
  
  const rect = canvas.getBoundingClientRect();
  const x = (e.clientX - rect.left) * (canvas.width / parseInt(canvas.style.width, 10));
  const y = (e.clientY - rect.top) * (canvas.height / parseInt(canvas.style.height, 10));
  
  const panelX = (canvas.width - 600) / 2;
  const panelY = (canvas.height - 400) / 2;
  const slotSize = 100;
  const slotY = panelY + 100;
  const slot1X = panelX + 100;
  ...
  // 슬롯 1에 드롭
  if (x >= slot1X && x <= slot1X + slotSize &&
      y >= slotY && y <= slotY + slotSize) {
    // 이미 슬롯에 있는 무기가 있다면 교체
    if (fusionUIState.slot1 === fusionUIState.draggedWeapon) {
      fusionUIState.slot1 = null;
    } else if (fusionUIState.slot2 === fusionUIState.draggedWeapon) {
      fusionUIState.slot2 = null;
      fusionUIState.slot1 = fusionUIState.draggedWeapon;
    } else {
      fusionUIState.slot1 = fusionUIState.draggedWeapon;
    }
  }
  // 슬롯 2에 드롭
  else if (x >= slot2X && x <= slot2X + slotSize &&
           y >= slotY && y <= slotY + slotSize) {
    if (fusionUIState.slot2 === fusionUIState.draggedWeapon) {
      fusionUIState.slot2 = null;
    } else if (fusionUIState.slot1 === fusionUIState.draggedWeapon) {
      fusionUIState.slot1 = null;
      fusionUIState.slot2 = fusionUIState.draggedWeapon;
    } else {
      fusionUIState.slot2 = fusionUIState.draggedWeapon;
    }
  }
  
  fusionUIState.draggedWeapon = null;
}

function handleFusionUIClick(x, y) {
  if (!fusionUIState.isOpen) return;
  ...
  if (x >= buttonX && x <= buttonX + buttonWidth &&
    ...
    if (fusionUIState.slot1 && fusionUIState.slot2) {
      const result = weaponFusionSystem.getFusionResult(
        fusionUIState.slot1.type,
        fusionUIState.slot2.type
      );
      
      if (result) {
        // 무기 제거
        const index1 = player.weapons.indexOf(fusionUIState.slot1);
        const index2 = player.weapons.indexOf(fusionUIState.slot2);
